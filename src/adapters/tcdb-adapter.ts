 ...
 *
 * NEO-38: TCDB is promoted to a first-class adapter conforming to the same
 * `class … extends BaseAdapter` form as the BSC and SportLots adapters, so the
 * Convex layer can dispatch BSC/SL/TCDB uniformly in a parallel fetch.
 *
 * TCDB is the canonical fallback source of set-level metadata (release date,
 * total card count, block/series) when BSC and SportLots responses come back
 * thin. TCDB is gated behind Cloudflare's "JavaScript required" interstitial —
 * plain HTTP fetch returns a challenge page, so we render via Puppeteer.
 *
 * Capabilities:
 *   - requiresAuth = false   — TCDB is fully public; no credentials/login.
 *   - supportsListing = false — read-only enrichment; we never push listings.
 *
 * Security: TCDB is a fully public site. No credentials are sent or stored.
 * No request/response field is logged that could contain user data — TCDB
 * enrichment is set-level catalog metadata only.
 */
...
// --- Module-scope scraping API ---------------------------------------------
//
// The class methods below delegate to these functions, which own their own
// Puppeteer browser lifecycle (launch + safeClose in finally). Keeping the
// proven, security-reviewed scraping logic here — rather than threading it
// through BaseAdapter.launchPage — means the class is a thin, uniform façade
// over the same code path that was already validated, and the pure helpers
// (scoreMatch / jaroWinkler / TCDB_MIN_SCORE) stay independently testable.
...
/**
 * First-class TCDB adapter conforming to the BaseAdapter contract.
 *
 * Unlike BSC/SportLots, TCDB is public: `login()` is a no-op that satisfies
 * the abstract contract without touching Secret Manager. The capability flags
 * let callers (and the Convex parallel-fetch dispatcher) introspect that TCDB
 * needs no auth and supports no listing/push.
 *
 * `search()` and `getSet()` are thin wrappers over the module-scope scraping
 * functions, which manage their own Puppeteer browser lifecycle. The
 * `page`/`launchPage`/`cleanup` machinery BaseAdapter provides for the
 * credentialed adapters is unused here — `cleanup()` is therefore always a
 * safe no-op (it only closes a browser the base class launched, and this
 * adapter never calls `launchPage`).
 */
export class TcdbAdapter extends BaseAdapter {
  /** TCDB is fully public — no credentials, no login. */
  readonly requiresAuth = false;
  /** Read-only enrichment source — NeonBinder never pushes listings to TCDB. */
  readonly supportsListing = false;

  constructor(page?: Page) {
    super(page, "The Trading Card Database (TCDB)");
  }
  ...
  /**
   * Public no-op login. TCDB requires no authentication; this exists solely
   * to satisfy the abstract BaseAdapter contract so the factory and the
   * Convex dispatcher can treat every adapter uniformly. The `key` argument
   * is ignored — no Secret Manager lookup is performed.
   */
  async login(_key?: string): Promise<AdapterResponse> {
    return { success: true, message: "TCDB is public (no auth)" };
  }

  /**
   * Search TCDB for sets matching a sport/year/setName triple. Returns
   * scored candidates (score ≥ TCDB_MIN_SCORE), highest first, capped at 10.
   * Throws `TcdbUnavailableError` on a Cloudflare challenge so the route
   * handler can degrade gracefully.
   */
  async search(query: TcdbSearchQuery): Promise<TcdbSetSearchResult[]> {
    return searchTcdbSets(query);
  }

  /**
   * Fetch set-level metadata for a numeric TCDB SID. Throws
   * `TcdbUnavailableError` on a Cloudflare challenge.
   */
  async getSet(tcdbSetId: string): Promise<TcdbSetMetadata> {
    return getTcdbSetMetadata(tcdbSetId);
  }
}
